...
# 페이지 기본 설정
st.set_page_config(page_title="Dooch XRL(F) 성능 곡선 뷰어 v3.8", layout="wide")
st.title("📊 Dooch XRL(F) 성능 곡선 뷰어 v3.8")
                    ...
                    if st.button("운전점 분석 실행"):
                        if not models:
                            ...
                            with st.spinner("선택된 모델들을 분석 중입니다..."):
                                # 분석 시 선택된 양정 컬럼(h_col_choice) 사용
                                if analysis_mode == "소방":
